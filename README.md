...
A single tetromino shape is spawned in the well and can be rotated using the keyboard. For clarity each cube of the piece is drawn with thick blue edges rather than a full wireframe surface, so no diagonal lines appear. Use the arrow keys to rotate the piece in different directions. This is an early step toward a complete 3D Tetris game.
